...
const ClientesDataContext = createContext(null);

export function useClientesData() {
  const ctx = useContext(ClientesDataContext);
  if (!ctx) throw new Error('useClientesData must be used inside ClientesDataProvider');
  return ctx;
}

export default function ClientesDataProvider({ children }) {
  ...
  return (
    <ClientesDataContext.Provider value={value}>
      {children}
    </ClientesDataContext.Provider>
  );
}
