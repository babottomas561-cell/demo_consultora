...
const LETRA_COLORS   = { A: '#0ea5e9', B: '#22c55e', C: '#f59e0b', E: '#8b5cf6', M: '#ec4899' };
const LETRA_LABEL    = { A: 'Fact. A', B: 'Fact. B', C: 'Fact. C', E: 'Exportac.', M: 'MiPyME', '?': 'Sin letra' };
const COND_PALETTE   = ['#4f46e5', '#16a34a', '#eab308', '#94a3b8'];
// condicion_venta_tipo arrives as a string from the backend cast
...
export default function ComprobantesTipoWidget() {
  ...
  const letraData = (data?.por_tipo_factura ?? [])
    .filter((r) => r.importe > 0)
    .map((r) => ({
      name: LETRA_LABEL[r.tipo] ?? `Fact. ${r.tipo}`,
      value: r.importe,
      color: LETRA_COLORS[r.tipo] ?? '#94a3b8',
    }));

  const condData = (data?.por_condicion_venta ?? [])
    .filter((r) => r.importe > 0)
    .map((r, i) => ({
      name: CONDICION_LABEL[String(r.condicion)] ?? r.condicion,
      value: r.importe,
  ...
  return (
    <div className="h-full w-full p-4 pt-0 flex gap-2 overflow-x-auto">
      {tipoData.length > 0  && <Donut chartData={tipoData}  title="Tipo comprobante" />}
      {letraData.length > 0 && <Donut chartData={letraData} title="Tipo factura (letra)" />}
      {condData.length > 0  && <Donut chartData={condData}  title="Condición de venta" />}
    </div>
  );
}
